...
exports.existeDocumento = existeDocumento;
exports.vincularCodigosSAP = vincularCodigosSAP;
exports.registrarOC = registrarOC;
exports.registrarRemito = registrarRemito;
exports.obtenerStockConsolidado = obtenerStockConsolidado;
exports.obtenerSaldosPorOC = obtenerSaldosPorOC;
const db_1 = require("./db");
/**
 * Verifica si un documento (OC o Remito) ya existe en el sistema.
 */
async function existeDocumento(numero) {
 ...
 */
async function vincularCodigosSAP(codigoCliente, codigoInterno, descripcion) {
    const db = await (0, db_1.getDatabase)();
    await db.run(`INSERT INTO mapeo_codigos_sap (codigo_sap_cliente, codigo_sap_interno, descripcion)
     ...
     ON CONFLICT(codigo_sap_cliente) DO UPDATE SET
       ...
       descripcion = COALESCE(excluded.descripcion, mapeo_codigos_sap.descripcion)`, [codigoCliente, codigoInterno, descripcion || null]);
}
/**
 * Registra una Orden de Compra y sus ítems de forma atómica.
 */
async function registrarOC(doc, items) {
    const db = await (0, db_1.getDatabase)();
    ...
    try {
        ...
        // Insertar documento
        await db.run('INSERT INTO documentos (numero, tipo, fecha, archivo_origen) VALUES (?, ?, ?, ?)', [doc.numero, 'OC', doc.fecha, doc.archivo_origen || '']);
        // Insertar items
        for (const item of items) {
            await db.run(`INSERT INTO items_oc (documento_numero, item_posicion, codigo_sap_cliente, descripcion, cantidad_original, saldo_pendiente, unidad)
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                doc.numero,
                item.item_posicion,
 ...
 */
async function registrarRemito(doc, items) {
    const db = await (0, db_1.getDatabase)();
    await db.run('BEGIN TRANSACTION');
    try {
        // Validar duplicado
        ...
        await db.run('INSERT INTO documentos (numero, tipo, fecha, archivo_origen) VALUES (?, ?, ?, ?)', [doc.numero, 'REMITO', doc.fecha, doc.archivo_origen || '']);
        for (const item of items) {
            // 1. Vincular los códigos SAP si vienen ambos en el remito
            if (item.codigo_sap_cliente && item.codigo_sap_interno) {
                await db.run(`INSERT INTO mapeo_codigos_sap (codigo_sap_cliente, codigo_sap_interno)
           VALUES (?, ?)
           ON CONFLICT(codigo_sap_cliente) DO UPDATE SET codigo_sap_interno = excluded.codigo_sap_interno`, [item.codigo_sap_cliente, item.codigo_sap_interno]);
            }
            ...
            const itemOC = await db.get('SELECT id, saldo_pendiente, descripcion FROM items_oc WHERE documento_numero = ? AND codigo_sap_cliente = ?', [item.oc_asociada_numero, item.codigo_sap_cliente]);
            if (!itemOC) {
                throw new Error(`El item con código SAP cliente ${item.codigo_sap_cliente} no existe en la OC asociada ${item.oc_asociada_numero}.`);
            }
            // 4. Validar saldo suficiente
            if (itemOC.saldo_pendiente < item.cantidad_entregada) {
                throw new Error(`Saldo insuficiente para el producto ${itemOC.descripcion} (${item.codigo_sap_cliente}) en la OC ${item.oc_asociada_numero}. Saldo actual: ${itemOC.saldo_pendiente}, Requerido: ${item.cantidad_entregada}`);
            }
            // 5. Descontar saldo de la OC
            const nuevoSaldo = itemOC.saldo_pendiente - item.cantidad_entregada;
            await db.run('UPDATE items_oc SET saldo_pendiente = ? WHERE id = ?', [nuevoSaldo, itemOC.id]);
            // 6. Registrar item del remito
            await db.run(`INSERT INTO items_remito (documento_numero, oc_asociada_numero, codigo_sap_interno, codigo_sap_cliente, cantidad_entregada, unidad)
         VALUES (?, ?, ?, ?, ?, ?)`, [
                doc.numero,
 ...
 */
async function obtenerStockConsolidado() {
    const db = await (0, db_1.getDatabase)();
 ...
 */
async function obtenerSaldosPorOC() {
    const db = await (0, db_1.getDatabase)();
    const rows = await db.all(`
    SELECT 
