  ...
  try {
    switch (name) {
      case 'procesar_documentos_pendientes': {
        ...
        if (!fs.existsSync(ocProcesadosDir)) fs.mkdirSync(ocProcesadosDir, { recursive: true });
        if (!fs.existsSync(remitosProcesadosDir)) fs.mkdirSync(remitosProcesadosDir, { recursive: true });
      ...
      case 'vincular_codigos_sap': {
        const { codigo_sap_cliente, codigo_sap_interno, descripcion } = args as {
      ...
      case 'cargar_saldos_iniciales_oc': {
        const { oc_numero, fecha, items } = args as {
          oc_numero: string;
          fecha: string;
          items: dao.ItemOC[];
        };
      ...
      case 'procesar_documento_por_ruta': {
        const { ruta_archivo, tipo } = args as { ruta_archivo: string; tipo: 'OC' | 'REMITO' };
        ...
        if (tipo === 'OC') {
          const extractedData = await parseOCWithGemini(ruta_archivo);
          await dao.registrarOC(
            {
              numero: extractedData.numero,
              tipo: 'OC',
          ...
          const extractedData = await parseRemitoWithGemini(ruta_archivo);
          const itemsToRegister = extractedData.items.map(item => ({
            documento_numero: extractedData.numero,
            oc_asociada_numero: extractedData.oc_asociada_numero,
            codigo_sap_interno: item.codigo_sap_interno,
